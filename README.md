...
装好即用，一条命令自检环境，一条命令看设备监测，全本地、零第三方依赖：

```bash
# 1. 部署自检：环境 / 本地模型 / 记忆库 一次看清
$ solo setup
{
  "checks": {
    "python": { "ok": true, "version": "3.11" },
    "ollama":  { "ok": true, "models": ["ornith:latest"] },
    "config":  { "ok": true, "has_model_config": true },
    "memory":  { "ok": true }
  }
}

# 2. 设备监测（内置演示数据）：自然语言问设备状态
$ solo monitor-demo
{
  "metrics": 164,
  "firing_alerts": 5,
  "q_high_temp": "以下设备temperature过高：d1 值=95.0, d2 值=95.0, t1 值=85.0",
  "q_max_temp": "temperature 最高的是 d1 值=95.0"
}
```

零依赖、本地跑、环境自检、设备状态一句话就能问。FDE 与一人公司的工具链，装好就能上手。
