...
### 新增(writing 优化器闭环, 吸收 qu-ai-wei 方法论)
- 门检 _gate_check: 改写前判真人文本, 真人停手不改声口(避免误改金庸/王朔)
- 语体识别 _detect_register: 9种语体, 避免把学术/公文误改口语
- optimize() 闭环: 检测→改写→复检, 输出 improvement 对比
- README 定位更新: 检测器→优化器(检测→改写→复检)
